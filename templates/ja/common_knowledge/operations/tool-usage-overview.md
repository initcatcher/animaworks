...
2. **MCP ツール (`mcp__aw__*`)**: AnimaWorks 固有の内部機能。`permissions.md` で許可された外部ツールも MCP 経由で直接利用可能
   - 内部: `send_message`, `post_channel`, `read_channel`, `read_dm_history`, `add_task`, `update_task`, `list_tasks`, `call_human`, `search_memory`, `report_procedure_outcome`, `report_knowledge_outcome`, `skill`, `disable_subordinate`, `enable_subordinate`, `set_subordinate_model`, `restart_subordinate`, `org_dashboard`, `ping_subordinate`, `read_subordinate_state`, `delegate_task`, `task_tracker`, `audit_subordinate`
...
1. **内部ツール**: `send_message`, `search_memory`, `read_file`, `execute_command`, `add_task` 等。ツール名をそのまま function calling で呼ぶ
2. **外部ツール**: `skill` ツールでツール名のスキルを確認し、CLI（`animaworks-tool <ツール> <サブコマンド>`）で実行
...
タスク管理・スーパーバイザー・refresh_tools・share_tool・create_anima は利用不可。外部ツールは `use_tool` で構造化呼び出し可能。
...
| ツール | 概要 |
|--------|------|
| `skill` | スキル・手続きの全文取得（外部ツールの使い方確認に使用） |
...
3. **長時間ツール**: 自動でバックグラウンド実行される（`submit` 相当）
